...
failureToExpectation f =
    Expect.fail (formatFailure f.description f.reason)
...
formatFailure : String -> Test.Runner.Failure.Reason -> String
formatFailure description reason =
    case reason of
...
failureWithStepsDescription passedSteps failedStep failure =
    passedSteps
        |> List.map (\s -> "✓ " ++ s)
        |> (\l -> l ++ [ "✗ " ++ failedStep ++ ":\n" ++ formatFailure failure.description failure.reason ])
        |> String.join "\n"
        |> failureFromDescription
